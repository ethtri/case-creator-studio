...
- Kexiaozhan 2026-06-09 Gmail attachments: local HTML trigger/proxy tools were provided for debugging only. Usage guide lists test domain `https://kxzcnt.kexiaozhan.com` and production domain `https://kxzus.kexiaozhan.com`; earlier docs/proxy default still mention `https://kxzsg.kexiaozhan.com`, so final sandbox/prod base URL still needs vendor confirmation.
