...
- Staging smoke on 2026-06-16 proved the signed Kexiaozhan-shaped redirect
  payload, Stripe test payment, webhook update, single onshore job, and dry-run
  signed Kexiaozhan callback metadata. It did not prove a live vendor callback
  because the test handoff was generated by Snapcase, not Kexiaozhan's sandbox
  order system.
